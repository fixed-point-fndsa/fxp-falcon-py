...
from target_construction import _b0_fft_fxp  # noqa: E402
...
def _prod_tree(sk, p):
    """Deployed keygen path at precision p: B0 (deployed `_b0_fft_fxp`,
    uncached so p-parametric) → _gram_fft_fxp → ffLDL root (the gram already
    emits g00@M_D / g10@M_G01 — no retag needed)."""
    return ffldl_fft_fxp_ntru_root(_gram_fft_fxp(_b0_fft_fxp(sk, p)), q=Q)
