...
class SecretKey:
    """
    Stateful wrapper around `Falcon(n).keygen()`. Exposes the
    attribute-based API used by the fxp module (`sk.f`, `sk.B0_fft`,
    `sk.sigma`, `sk.hash_to_point()`, etc.) on top of the modern
    `Falcon` manager class. Both APIs cohabit in this file:

      - `Falcon(n).keygen() -> (sk_tuple, vk_bytes)` is the canonical
    ...
    def __init__(self, n: int, polys: list[list[int]] | None = None):
        self._falcon = Falcon(n)
        sk_tuple, _vk = self._falcon.keygen(polys)
        (self.f, self.g, self.F, self.G,
         self.B0_fft, self.T_fft) = sk_tuple
        # Public scalar parameters — same names as upstream pre-refactor.
        self.n = n
        self.sigma = self._falcon.param.sigma
        self.sigmin = self._falcon.param.sigmin
        self.signature_bound = self._falcon.param.sig_bound
        self.sig_bytelen = self._falcon.param.sig_bytelen
        # Public key polynomial h = g/f mod q (matches upstream pre-refactor;
        # the Falcon manager keeps `vk` serialized but the fxp module uses h).
        self.h = div_zq(self.g, self.f)
        # Per-key fxp caches, populated lazily by the fxp module on first
        # use. Declaring them here (rather than via dynamic `setattr`) makes
        # the contract explicit and protects against typos at access sites.
        self._B0_fft_fxp = None    # set by target_construction._build_B0_fft_fxp_cache
        self._fxp_tree = None      # set by sign_tweak._build_fxp_tree_cache

    def hash_to_point(self, message: bytes, salt: bytes) -> list[int]:
        return self._falcon.__hash_to_point__(message, salt)

    def sample_preimage(self, point, seed=None):
        return self._falcon.__sample_preimage__(
            self.B0_fft, self.T_fft, point, seed)

    def sign(self, message: bytes, randombytes=urandom) -> bytes:
        sk_tuple = (self.f, self.g, self.F, self.G,
                    self.B0_fft, self.T_fft)
        return self._falcon.sign(sk_tuple, message, randombytes)

    def verify(self, message: bytes, signature: bytes) -> bool:
        return self._falcon.verify(
            serialize_poly(self.h), message, signature)
